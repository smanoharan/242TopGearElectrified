...
namespace GeneratedGeometryPipeline
{
    ...
    {
        const float cylinderSize = 2;

        const int cylinderSegments = 64;
        
        const float texCoordTop = 0.01f;
        const float texCoordBottom = 0.94f;
        const float textCoordReflect = 0.99f;
                                           ...
                                           ContentProcessorContext context)
        {
            ...
            string effect = Path.Combine(directory, "sky.fx");
            ...
            for (int i = 0; i < cylinderSegments; i++)
            {
                ...
                //AddVertex(builder, bottomVertices[i], texCoordId, u1, texCoordBottom);
                //AddVertex(builder, bottomVertices[j], texCoordId, u2, texCoordBottom);
                AddVertex(builder, bottomCenterVertex, texCoordId, u2, textCoordReflect);
                AddVertex(builder, bottomVertices[i], texCoordId, u1, texCoordBottom);
                AddVertex(builder, bottomVertices[j], texCoordId, u2, texCoordBottom);
            }
